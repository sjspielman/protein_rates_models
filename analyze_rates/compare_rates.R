...
rates %>% 
    ...
    group_by(type) %>%
    spread(model, MLE) %>%
    gather(model, MLE, WAG:JC69) -> model.scatter.data
...
all.corr %>%
    ...
    facet_grid(~type) + 
    xlab("") + ylab("") + 
    theme(axis.text.x = element_text(angle=15, size=7.5), axis.text.y = element_text(size=8)) -> heatmap.rho.between.models
ggsave(paste0(figdir, "heatmap_correlations.pdf"), heatmap.rho.between.models, width=11.25, height=2.8)    
