...
# counts <- bind_rows(enzyme, org, gpcr) %>% 
#             filter(subtype == "total") %>%
#             select(-subtype)
# counts$model <- factor(counts$model, levels = model.levels, labels = model.labels)
# counts$type <- factor(counts$type, levels = type.levels, labels = type.labels)
# 
# rates.norv <- rates %>%
#                 filter(rv == "No") %>%
#                 select(-rv, -Lower, -Upper, -LogL_global, -LogL_local)
...
# 
# counts %>% 
#     left_join(rates.norv) %>%
#     group_by(model, dataset, type) %>%
#     summarize( rho = cor(MLE, count, method = "spearman")) -> rate.count.rho
# 
# plot.ratecount <- function(df, mytype){
# 
#     df %>% 
#         ungroup() %>%
#         filter(type == mytype) -> x
#     x %>% 
#         group_by(dataset) %>% 
#         filter(rho == min(rho)) %>% 
#         arrange(rho) %>% 
...
#         select(dataset, n) %>%
#         left_join(x) %>%        
#             ggplot(aes(x = n, y=rho, group=model, color=model)) + 
#             geom_path(alpha=0.8) +
#             xlab("Alignment, no particular order") + ylab("Rho") + 
#             panel_border() + 
#             ggtitle(mytype) -> p
#     p
# }
# 
# enz <- plot.ratecount(rate.count.rho, "Enzyme")
# leg <- get_legend(enz)
# enz + theme(legend.position = "none") -> enz
# mit <- plot.ratecount(rate.count.rho, "Mitochondria") + theme(legend.position = "none")
# chl <- plot.ratecount(rate.count.rho, "Chloroplast")+ theme(legend.position = "none")
# gpcr <- plot.ratecount(rate.count.rho, "GPCR")+ theme(legend.position = "none")
# 
# plot_grid(enz, mit, chl, gpcr,nrow=2) -> raw
# plot_grid(raw, leg, nrow=1, rel_widths=c(1,0.1)) -> plot.raterho
# save_plot("rates_count_spearman.pdf", plot.raterho, base_width=9, base_height=4)
# 
# 
# 
# lowdats <- c("1GA8_A", "1PMA_B", "1YON_A", "1OS7_A")
# highdats <- c("1EG7_A", "1TLP_E", "1TYF_A", "1MHY_D")
# 
# counts %>% 
#     left_join(rates.norv) %>%
#     filter(dataset %in% lowdats) %>%
#     filter(MLE >0) -> low
# 
# low %>% 
#     filter(model %in% c("LG", "JC")) %>%
#     ggplot(aes(x = count,y = MLE)) + 
...
# 
# 
# ggsave("lowrho.pdf", lows)
# ggsave("highrho.pdf", highs)
# 
