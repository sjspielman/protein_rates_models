...
leisr.constraint  = io.PromptUser ("\n>Value for bl constraint?",1,0,100000000,FALSE);
/*******************************************************************************************************************/
...
/* CONSTRAIN AS SPECIFIED */
if (leisr.constraint_choice == "input"){
        // fix to input branch lengths
        leisr.initial_bl = parameters.helper.tree_lengths_to_initial_values (leisr.trees, None);
} else  {
    
    leisr.initial_bl = utility.Map (leisr.partitions_and_trees, "_value_", "(_value_[terms.data.tree])[terms.trees.partitioned]");                          
    ...
    if (leisr.constraint_choice == "value"){
        // some provided value
        utility.ForEachPair (leisr.initial_bl, "_key_","_value_", "
            leisr.initial_bl[key] = utility.Map (_value_, '_v2_', '{terms.fit.MLE : leisr.constraint}');
        ");
        }
    leisr.initial_bl = { terms.branch_length : leisr.initial_bl};        
};
...
leisr.alignment_wide_MLES = estimators.FitSingleModel_Ext(
                                                          ...
                                                          {terms.run_options.proportional_branch_length_scaler : 
                                                            {"0" : terms.model.branch_length_constrain}, 
